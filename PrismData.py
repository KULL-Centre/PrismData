# Copyright (C) 2020-2023 Kristoffer Enoe Johansson <kristoffer.johansson@bio.ku.dk>
...
class VariantData(PrismData):
    ...
    def to_new_reference(self, target_seq=None, first_resn=None, verbose=0,
        ...
        if not target_seq is None:
            ...
            if len(alignments) > 1:
                ...
                max_alignments = 10
                alignments_scores = np.array([alignments[i].score for i in range(min(len(alignments),max_alignments))])
            ...
            n_aligned_pos = n_match+n_mismatch
            coverage = n_aligned_pos/n_res_target
            if coverage < min_coverage:
                # raise PrismValueFail("Sequence coverage %.3f (%d/%d) below threshold of %.3f" % (coverage, n_res-n_indel, n_res, min_coverage))
                raise PrismValueFail("Sequence coverage %.3f (%d/%d) below threshold of %.3f" % (coverage, n_aligned_pos, n_res_target, min_coverage))
            # identity = (n_res_target-n_mismatch)/n_res_target
            identity = n_match/n_aligned_pos
    ...
    def merge(self, data_list, target_seq=None, first_resn=None, merge='outer', **kwargs):
        ...
        **kwargs : keyword arguments
        ...
        # Add the index columns ['n_mut','aa_ref','resi','aa_mut'] again and order variants
        merged_data.add_index_columns()
        merged_data.order_variants(aa_order="ACDEFGHIKLMNPQRSTVWY=*~")
...
if __name__ == "__main__":
    ...
    if not (int(bio_version_list[0]) >= 1 and int(bio_version_list[0]) >= 80):
